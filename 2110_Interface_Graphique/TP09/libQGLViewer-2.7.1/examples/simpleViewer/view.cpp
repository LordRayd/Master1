...
void View::draw() {
                                ...
                                fit != parser.faceArray.end(); fit++)
    {
        ...
        glTexCoord2f((*fit)->u1,(*fit)->v1);

        glNormal3dv((*fit)->p1->normal);
        glVertex3dv(P1);

        glTexCoord2f((*fit)->u2,(*fit)->v2);

        glNormal3dv((*fit)->p2->normal);
        glVertex3dv(P2);

        glTexCoord2f((*fit)->u3,(*fit)->v3);
...
void View::init() {
    ...
    QImage texture = QImage("./giraffe_skin.jpg");
    QImage glImg = QGLWidget::convertToGLFormat(texture);
    // flipped 32bit RGBA
